...
def assign_speakers(
    ...
    For each segment, find the diarization interval with the greatest
    temporal overlap and copy its speaker label. If diarization is
    ...
    for seg in segments:
        ...
        for diar in diarization:
            diar_start = diar.get("start_s", 0.0)
            diar_end = diar.get("end_s", 0.0)
