...
async def download_endpoint(video_id: str):
    """Download video and captions, returning video_id and caption segments."""
    url = f"https://www.youtube.com/watch?v={video_id}"
    video_id, title = _download_service.get_video_info(url)
    ...
    if not video_path.exists():
        _download_service.download_video(url, str(videos_dir), stem)

    if not caption_path.exists():
        _download_service.download_caption(url, str(captions_dir), stem)
